...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const whereClause =
      q.length > 0
        ? or(ilike(systemUser.name, `%${q}%`), ilike(systemUser.email, `%${q}%`))
        : undefined;
...
export async function POST(req: NextRequest) {
  const authError = await requireAnyPermission(req, [Permission.CREATE_SYSTEM_USER]);
  if (authError) return authError;
  ...
  try {
    ...
    // TODO: send welcome/password-reset email here
